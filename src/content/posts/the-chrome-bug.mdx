...
description: "סיפור על גילוי באג מוזר בכרומיום שמקריס את הטאב - באג שקיים מגרסה 91, אבל כמעט אף אחד לא נתקל בו בגלל השילוב המיוחד של תנאים שצריכים להתקיים."
---
...
```
יצרנו דוגמה פשוטה. הרצנו. לא קרס.
...
הייתי בטוח שאני מפספס משהו, אבל לא ידעתי מה. הקוד הוא די גדול ויכולים להיות הרבה חלקים שגורמים לבאג.

## פריצת הדרך

בסוף החלטתי לתת ל-cursor את כל הקוד של ה-iframe.

סוף סוף הוא יצר דוגמה מינימלית שמשחזרת את הבאג, עברתי עליה והורדתי עוד כמה דברים מיותרים, עד שהגענו לדוגמה **המינימלית לחלוטין** שמשחזרת את הבאג:
...
6. הפונקציה preventDefault() מנסה לעצור את התנהגות ברירת המחדל של הכפתור.
...
נדיר מאוד למצוא קוד מודרני שעובד בצורה כזו.
### 4. קריאה ישירה לפונקציה של ההורה


ולכן הבאג הזה חי בשקט כבר 4 שנים.

## הדיווח ל-Chromium
כדי לוודא שהבאג לא קורה רק אצלי, השתמשתי ב-[Browsers Stack](https://live.browserstack.com/dashboard) כדי לבדוק גרסאות כרום ו-סביבות שונות.
מסתבר שהבאג התחיל בגרסה 91 ומשתחזר רק במחשבי Mac.
...
מן הסתם יקחו כמה חודשים עד שייכנס תיקון ועד
## ומה למדתי?
